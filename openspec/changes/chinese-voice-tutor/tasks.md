...
- [ ] 1.1 Add `REALTIME_VOICE` (and any realtime model id) to env handling and document required vars in README/`.env.local` example
- [ ] 1.2 Initialize shadcn/ui targeting Base UI with the neutral base color, scoped so it does not disturb existing pages; verify it builds with Tailwind v4
- [ ] 1.3 Add the shadcn primitives the tutor needs (button, card, toggle, and a select/segmented control for the sensitivity level) under `src/components/ui/`
...
- [ ] 2.1 Create `src/lib/tutor/types.ts`: conversation entry types (utterance, correction, learner turn), session state enum (idle/connecting/listening/thinking/speaking/error), sensitivity level type (LOW/MEDIUM/HIGH)
- [ ] 2.2 Create `src/lib/tutor/prompt.ts`: system prompt builder taking a sensitivity level, encoding the 李老师 persona, Chinese-only ordinary speech, mixed-language verbal correction explanations, HSK2 baseline, LOW/MEDIUM/HIGH thresholds for pronunciation + grammar, and the tool-usage rules
...
- [ ] 3.1 Create `POST /api/tutor/session/route.ts` following existing route conventions (Stack auth → 401 if unauthenticated, `maxDuration = 60`, JSON response)
- [ ] 3.2 Mint a short-lived realtime session token server-side using the existing OpenAI key; return `{ token, model, voice }` and never expose the long-lived key
- [ ] 3.3 Add a test covering the unauthenticated 401 path and the shape of a successful response

## 4. Provider abstraction

- [ ] 4.1 Create `src/lib/tutor/provider.ts`: define the `RealtimeProvider` interface (mint-credentials contract, build session config, interpret events/tool-calls/state)
...
- [ ] 5.1 Create `src/lib/tutor/useTutorSession.ts`: fetch ephemeral token, set up `RTCPeerConnection`, capture mic via `getUserMedia`, attach remote audio track to an `<audio>` element started on the user gesture
- [ ] 5.2 Open the `oai-events` data channel and send `session.update` with modalities `["text","audio"]`, `server_vad` turn detection (barge-in), persona/system prompt, and tool definitions
- [ ] 5.3 Parse data-channel events into typed conversation entries (`display_utterance`, `show_correction`), append learner turns from input transcription when available, and fall back to transcript text if a tutor turn emits no tool call
- [ ] 5.4 Drive the session state machine (idle → connecting → listening ↔ thinking ↔ speaking, error) and expose start/stop, mute, and reconnect actions; stopping releases the mic and closes the connection
...
- [ ] 6.5 Build the inline correction card (amber styling `bg-amber-50 border-l-4 border-amber-400`, dark `bg-amber-950/30 border-amber-500`) rendered in the thread flow, showing target Hanzi/pinyin, the `description`, and the retry label; covers pronunciation and grammar, and retains its styling in scrollback
